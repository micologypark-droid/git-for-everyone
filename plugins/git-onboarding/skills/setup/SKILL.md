...
7. `test -x .git/hooks/pre-commit && echo "pre-commit: exists" || echo "pre-commit: none"; test -x .git/hooks/pre-push && echo "pre-push: exists" || echo "pre-push: none"` — git hooks 설치 여부
...
## 단계 7: 메인 브랜치 보호 (git hooks)

**완료 조건:** `.git/hooks/pre-commit`과 `.git/hooks/pre-push` 파일이 모두 존재하고 실행 권한이 있음

아래 명령어로 확인하세요:

```bash
test -x .git/hooks/pre-commit && echo "pre-commit: exists" || echo "pre-commit: none"; test -x .git/hooks/pre-push && echo "pre-push: exists" || echo "pre-push: none"
```

### pre-commit hook

main/master에서 직접 commit을 차단합니다. 없는 경우 아래 내용으로 `.git/hooks/pre-commit` 파일을 생성하세요:

```bash
#!/bin/sh

# Prevent direct commits on protected branches (main/master)

branch=$(git symbolic-ref --short HEAD 2>/dev/null)

if [ "$branch" = "main" ] || [ "$branch" = "master" ]; then
  echo ""
  echo "[BLOCKED] '$branch' 브랜치에서 직접 commit할 수 없습니다."
  echo ""
  echo "  새 브랜치를 만들어서 작업하세요:"
  echo "    git checkout -b feat/<작업-설명>"
  echo ""
  echo "  변경 사항은 새 브랜치로 자동으로 옮겨집니다."
  echo ""
  exit 1
fi
```
...
main/master로 직접 push를 차단합니다. 없는 경우 아래 내용으로 `.git/hooks/pre-push` 파일을 생성하세요:

```bash
#!/bin/sh

# Prevent direct push to protected branches (main/master)

while read local_ref local_sha remote_ref remote_sha; do
  if echo "$remote_ref" | grep -qE "refs/heads/(main|master)$"; then
    branch=$(echo "$remote_ref" | sed 's|refs/heads/||')
    echo ""
    echo "[BLOCKED] '$branch' 브랜치에 직접 push할 수 없습니다."
    echo ""
    echo "  새 브랜치를 만들어서 작업하세요:"
    echo "    git checkout -b <브랜치-이름>"
    echo "    git push -u origin <브랜치-이름>"
    echo ""
    exit 1
  fi
done
...
Git hook을 설치했습니다.
  ...
  pre-commit: main/master에서 commit 시도 시 차단
  pre-push:   main/master로 push 시도 시 차단

이제 새 브랜치를 만들어서 작업해야 합니다.
...
Git 초기 설정이 완료되었습니다.
  ...
  GitHub:  로그인됨
  저장소:  <현재 디렉토리>
  보호:    main/master commit/push 차단 (pre-commit + pre-push hook)
